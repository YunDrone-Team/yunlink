...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Python dual-UAV air-side routing helper.")
...
def main() -> int:
    ...
    try:
        lease1 = wait_for_lease(uav1, timeout_s=4.0)
        lease2 = wait_for_lease(uav2, timeout_s=4.0)
        ...
        uav2.publish_vehicle_core_state(
            lease2.peer,
            TargetSelector.entity(AgentType.GROUND_STATION, 999),
            VehicleCoreState(True, 4, 21.0, 22.0, 23.0, 0.1, 0.2, 0.3, 91.0),
            lease2.session_id,
        )
        time.sleep(0.2)
        uav2.publish_vehicle_core_state(
        ...
        wait_for_release(uav2, lease2.session_id, timeout_s=4.0)
        wait_for_release(uav1, lease1.session_id, timeout_s=4.0)
        print("python-air-dual-uav ok")
        return 0
    finally:
        uav2.close()
        uav1.close()
