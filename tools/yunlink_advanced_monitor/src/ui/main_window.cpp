...
QTableWidget* MainWindow::create_topic_table(QWidget* parent) {
    ...
    table->setHorizontalHeaderLabels({"字段说明", "字段键 (Field key)", "YunLink 最新值"});
...
QWidget* MainWindow::build_dashboard_page(QWidget* parent) {
    ...
    auto* status_group = new QGroupBox("运行状态", content);
    ...
    status_grid->addWidget(new QLabel("控制权", status_group), 0, 2);
    ...
    status_grid->addWidget(new QLabel("对端", status_group), 2, 0);
    status_grid->addWidget(remote_value_, 2, 1);
    status_grid->addWidget(new QLabel("本地监听", status_group), 2, 2);
    ...
    auto* dashboard_grid = new QGridLayout();
    dashboard_grid->setHorizontalSpacing(12);
    dashboard_grid->setVerticalSpacing(12);
    dashboard_grid->addWidget(
        build_dashboard_card(content, "YunLink 链路", &dashboard_yunlink_summary_, &dashboard_yunlink_body_),
        0,
        0);
    dashboard_grid->addWidget(
        build_dashboard_card(content, "PX4", &dashboard_px4_summary_, &dashboard_px4_body_), 0, 1);
    dashboard_grid->addWidget(build_dashboard_card(content,
                                                   "定位融合",
    ...
    auto* issues_group = new QGroupBox("阻塞问题", content);
    ...
    dashboard_grid->addWidget(issues_group, 2, 1);
