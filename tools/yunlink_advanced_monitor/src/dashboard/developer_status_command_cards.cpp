...
DeveloperStatusCard build_control_card(const MonitorTopicState* control_topic,
    ...
    card.summary.title = "Control";
    if (control_topic == nullptr || !monitor_has_snapshot(control_topic->latest)) {
        card.summary = make_line(DeveloperStatusLevel::kWarn, "Control", "uav_control_state WAIT");
        add_issue(issues, card.summary.level, "Control", card.summary.detail);
    ...
    if (topic_bool(values, "odometry_lost") || !topic_bool(values, "odometry_valid")) {
        ...
        add_issue(issues, level, "Control", detail);
    } else {
    ...
    card.summary = make_line(level, "Control", detail);
    push_row(card.rows,
             "agent",
    ...
    push_row(card.rows, "self odom", xyz_text(values, "self_odom.pose.position_m"));
    push_row(card.rows, "self att", euler_text(values, "self_odom.pose."));
    push_row(card.rows, "age", snapshot_age_text(control_topic, now_ms));
    return card;
}
...
DeveloperStatusCard build_command_card(const MonitorTopicState* cmd_topic,
    ...
    if (!history.empty()) {
        ...
        if (last.lifecycle == MonitorCommandLifecycle::kFailed ||
            ...
            add_issue(issues, level, "Command", detail);
        } else {
            level = command_lifecycle_is_terminal(last.lifecycle) ? DeveloperStatusLevel::kOk
                                                                  : DeveloperStatusLevel::kWarn;
            detail = last.action + " " + command_lifecycle_label(last.lifecycle);
        }
        push_row(card.rows, "history", detail);
    } else {
        push_row(card.rows, "history", "无 command history");
    }
    ...
    if (diagnostic_topic != nullptr && monitor_has_snapshot(diagnostic_topic->latest)) {
        ...
        if (cmd_status_level != DeveloperStatusLevel::kOk) {
            ...
            add_issue(issues, cmd_status_level, "Command", "uav_control_cmd: " + cmd_status);
        }
    } else {
        push_row(card.rows, "raw status", "WAIT");
    }
    ...
    card.summary = make_line(level, "Command", detail);
    return card;
}
