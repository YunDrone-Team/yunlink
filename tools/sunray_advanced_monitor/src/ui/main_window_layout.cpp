...
QWidget* MainWindow::build_status_panel(QWidget* parent) {
    ...
    grid->addWidget(note_label, 2, 0);
    grid->addWidget(note_value_, 2, 1, 1, 5);
...
QWidget* MainWindow::build_command_panel(QWidget* parent) {
    ...
    body_point_x_spin_ = make_spin(0.0, -50.0, 50.0, 0.1);
    body_point_y_spin_ = make_spin(0.0, -50.0, 50.0, 0.1);
    body_point_z_spin_ = make_spin(1.5, -10.0, 100.0, 0.1);
    body_point_yaw_spin_ = make_spin(0.0, -3.14159, 3.14159, 0.1, 3);
    ...
    body_vx_spin_ = make_spin(0.0, -5.0, 5.0, 0.1);
    body_vy_spin_ = make_spin(0.0, -5.0, 5.0, 0.1);
    body_height_spin_ = make_spin(1.5, -10.0, 100.0, 0.1);
    body_yaw_rate_spin_ = make_spin(0.0, -3.14159, 3.14159, 0.1, 3);
    ...
    body_point_button_ = new QPushButton("预备 MOVE_POINT_BODY", group);
    velocity_button_ = new QPushButton("预备 MOVE_VELOCITY", group);
    body_velocity_button_ = new QPushButton("预备 MOVE_VELOCITY_BODY", group);
    ...
    connect(body_point_button_, &QPushButton::clicked, this, &MainWindow::stage_move_point_body);
    connect(velocity_button_, &QPushButton::clicked, this, [this]() { stage_move_velocity(false); });
    connect(body_velocity_button_, &QPushButton::clicked, this, [this]() { stage_move_velocity(true); });
    ...
    layout->addWidget(new QLabel("机体系位置 x/y/z/yaw(rad)"), 3, 0);
    layout->addWidget(body_point_x_spin_, 3, 1);
    layout->addWidget(body_point_y_spin_, 3, 2);
    layout->addWidget(body_point_z_spin_, 3, 3);
    layout->addWidget(body_point_yaw_spin_, 3, 4);
    layout->addWidget(body_point_button_, 3, 5);

    layout->addWidget(new QLabel("惯性系速度 vx/vy/vz/yaw_rate"), 4, 0);
    layout->addWidget(vel_x_spin_, 4, 1);
    layout->addWidget(vel_y_spin_, 4, 2);
    layout->addWidget(vel_z_spin_, 4, 3);
    layout->addWidget(vel_yaw_rate_spin_, 4, 4);
    layout->addWidget(velocity_button_, 4, 5);

    layout->addWidget(new QLabel("机体系速度 vx/vy/height/yaw_rate"), 5, 0);
    layout->addWidget(body_vx_spin_, 5, 1);
    layout->addWidget(body_vy_spin_, 5, 2);
    layout->addWidget(body_height_spin_, 5, 3);
    layout->addWidget(body_yaw_rate_spin_, 5, 4);
    layout->addWidget(body_velocity_button_, 5, 5);

    layout->addWidget(command_hint_label_, 6, 0, 1, 6);
    return group;
}
...
QWidget* MainWindow::build_command_history_panel(QWidget* parent) {
    ...
    command_history_table_->setHorizontalHeaderLabels(
        {"时间", "命令", "状态", "Session", "Message ID", "回执", "回显"});
