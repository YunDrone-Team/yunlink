...
void AdvancedMonitorBackend::on_odom_state(const sunray_msgs::OdomState::ConstPtr& msg) {
    std::unordered_map<std::string, std::string> values;
    fill_odom_state_from_ros(*msg, values);
    update_ros("odom_state", std::move(values), msg->header.stamp);
}

void AdvancedMonitorBackend::on_control_state(const sunray_msgs::UAVControlState::ConstPtr& msg) {
    std::unordered_map<std::string, std::string> values;
...
void AdvancedMonitorBackend::update_ros(const std::string& key,
                                        std::unordered_map<std::string, std::string>&& values,
    ...
    auto& snapshot = it->second.ros_latest;
...
void AdvancedMonitorBackend::on_reconnect_timer(const ros::TimerEvent&) {
    ...
    log(MonitorLogLevel::kInfo,
        MonitorLogSource::kSession,
        "已连接 yunros，对端 peer_id=" + peer_id + "，session_id=" +
