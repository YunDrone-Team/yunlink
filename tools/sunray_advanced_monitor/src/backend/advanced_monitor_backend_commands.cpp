...
void AdvancedMonitorBackend::stage_command(const MonitorCommandDraft& draft) {
    if (!draft.valid) {
        log(MonitorLogLevel::kWarn, MonitorLogSource::kRuntime, "忽略无效命令草稿");
        return;
    }

    const bool ready = can_send_commands();
    const std::string prefix =
        ready ? "命令已预备（模块4仅写日志）: " : "命令已填写，但当前 session 未就绪: ";
    std::string line = prefix + draft.summary;
    if (!draft.detail.empty()) {
        line += " | " + draft.detail;
    }
    line += draft.continuous ? " | mode=continuous-ui-only" : " | mode=single-ui-only";
    log(ready ? MonitorLogLevel::kInfo : MonitorLogLevel::kWarn,
...
void AdvancedMonitorBackend::send_unsupported_command(const MonitorCommandDraft& draft) {
    std::string line = "命令暂未接入 YunLink 发送: " + draft.summary;
    if (!draft.detail.empty()) {
        line += " | " + draft.detail;
    }
    log(MonitorLogLevel::kWarn, MonitorLogSource::kSession, line);
}
