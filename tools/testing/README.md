...
每次执行还会生成 `suite-manifest.json` 与逐 case `cases/*.json`，供 `report/` 和 `perf/` 继续消费。
...
- `dual-host-baseline` 最新产物位于 `output/testing/20260428-175748/dual-host-baseline/`，三条基线 case 都会写出非零 `connect/session/authority/command/state` 指标。
- `dual-host-recovery` 最新产物位于 `output/testing/20260428-175102/dual-host-recovery/`，authority expiry 与 peer disconnect 两条新增恢复 case 都会写出非零 `recovery_ms`。
- `dual-host-competition` 最新带非零 metrics 的产物位于 `output/testing/20260428-175805/dual-host-competition/`。
- `dual-host-routing` 最新带非零 metrics 的产物位于 `output/testing/20260428-175825/dual-host-routing/`。
- `dual-host-scale` 最新产物位于 `output/testing/20260428-175153/dual-host-scale/`，验证 `2 GCS -> 2 UAV` 并输出六类固定 metrics。
