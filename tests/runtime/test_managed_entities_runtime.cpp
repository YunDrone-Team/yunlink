...
int main() {
    ...
    air.system_service_subscriber().subscribe_managed_entity_list_requests(
        [&](const yunlink::InboundSystemServiceRequestView<yunlink::ManagedEntityListRequest>& view) {
    ...
    if (air.publish_state_from(uav(2),
                               air_session.peer.id,
                               yunlink::TargetSelector::for_entity(
                                   yunlink::AgentType::kGroundStation, 1001),
                               state,
    ...
    if (air.publish_state_from(uav(99),
                               air_session.peer.id,
                               yunlink::TargetSelector::for_entity(
                                   yunlink::AgentType::kGroundStation, 1001),
                               state,
    ...
    if (air.publish_state_from(uav(3),
                               air_session.peer.id,
                               yunlink::TargetSelector::for_entity(
                                   yunlink::AgentType::kGroundStation, 1001),
                               state,
    ...
    if (air.set_managed_identities({uav(3), uav(3)}) !=
            yunlink::ErrorCode::kInvalidArgument ||
