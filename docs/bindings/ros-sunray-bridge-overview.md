...
这也是为什么一期 bridge 包放在 `sunray_v2/communication/sunray_yunlink_bridge/`，但官方 overview 放在 `yunlink/docs/bindings/ros-sunray-bridge-overview.md`：实现归 Sunray 工作区，协议主导权与官方集成说明归 Yunlink。

## 系统边界

- `yunlink` 仓库新增 runtime 外部命令处理模式。
  bridge 使用 `CommandHandlingMode::kExternalHandler`，关闭默认 `runtime-auto-result`。
- `sunray_v2/communication/sunray_yunlink_bridge/` 是独立 catkin 包、独立进程。
...
| --- | --- | --- |
| 无 active session | 直接 `Failed(no-active-session)` | 否 |
| 无 authority | 直接 `Failed(no-authority)` | 否 |
| TTL 过期 | 直接 `Failed(ttl-expired)` | 否 |
