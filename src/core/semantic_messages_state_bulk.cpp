...
bool decode_payload(const ByteBuffer& bytes, Px4StateSnapshot* payload) {
    return parse_payload(bytes, payload, [](BufferReader& reader, Px4StateSnapshot* out) {
        return reader.read_bool(&out->connected) && reader.read_bool(&out->rc_available) &&
               ...
               reader.read_u8(&out->landed_state) &&
               reader.read_float(&out->battery_voltage_v) &&
...
bool decode_payload(const ByteBuffer& bytes, LocalOdomSnapshot* payload) {
    return parse_payload(bytes, payload, [](BufferReader& reader, LocalOdomSnapshot* out) {
        return read_vec3(reader, &out->position_m) &&
               reader.read_float(&out->orientation_x) && reader.read_float(&out->orientation_y) &&
               reader.read_float(&out->orientation_z) && reader.read_float(&out->orientation_w) &&
...
bool decode_payload(const ByteBuffer& bytes, UavControlStateSnapshot* payload) {
    return parse_payload(bytes, payload, [](BufferReader& reader, UavControlStateSnapshot* out) {
        return reader.read_u8(&out->controller_types) &&
               reader.read_double(&out->takeoff_relative_height_m) && reader.read_double(&out->takeoff_max_velocity_mps) &&
               reader.read_u8(&out->land_type) && reader.read_double(&out->land_max_velocity_mps) &&
...
bool decode_payload(const ByteBuffer& bytes, OdomStateSnapshot* payload) {
    return parse_payload(bytes, payload, [](BufferReader& reader, OdomStateSnapshot* out) {
        return reader.read_u8(&out->external_source) && reader.read_string(&out->subtopic_name_external_odom) &&
               reader.read_bool(&out->odometry_valid) && reader.read_float(&out->odometry_update_hz) &&
               reader.read_string(&out->subtopic_name_external_relocalization) && reader.read_string(&out->pubtopic_name_local_odom) &&
               reader.read_string(&out->pubtopic_name_global_odom) && reader.read_string(&out->world_frame_name) &&
               reader.read_string(&out->global_frame_name) && reader.read_string(&out->local_frame_name) &&
